"""
    Modified script for SQuAD version 2.0,
    original: https://github.com/huggingface/datasets/blob/master/metrics/squad_v2/evaluate.py
"""
import argparse
import collections
import json
...
def get_precision_recall_f1(a_gold, a_pred, metric_type="f1"):
...
def compute_f1(a_gold, a_pred):
    return get_precision_recall_f1(a_gold, a_pred, metric_type="f1")


def compute_precision(a_gold, a_pred):
    return get_precision_recall_f1(a_gold, a_pred, metric_type="precision")


def compute_recall(a_gold, a_pred):
    return get_precision_recall_f1(a_gold, a_pred, metric_type="recall")
...
def make_eval_dict(
    exact_scores, f1_scores, precision_scores, recall_scores, qid_list=None
):
    if not qid_list:
        ...
        return collections.OrderedDict(
            [
                ...
                (
                    "precision",
                    100.0 * sum(precision_scores[k] for k in qid_list) / total,
                ),
...
def find_all_best_thresh(
    ...
    best_precision, precision_thresh = find_best_thresh(
        preds, precision_raw, na_probs, qid_to_has_ans
    )
    best_recall, recall_thresh = find_best_thresh(
        preds, recall_raw, na_probs, qid_to_has_ans
    )
    main_eval["best_exact"] = best_exact
    main_eval["best_exact_thresh"] = exact_thresh
    main_eval["best_f1"] = best_f1
    main_eval["best_f1_thresh"] = f1_thresh
    main_eval["best_precision"] = best_precision
    main_eval["best_precision_thresh"] = precision_thresh
    main_eval["best_recall"] = best_recall
    main_eval["best_recall_thresh"] = recall_thresh
...
def main():
    ...
    precision_thresh = apply_no_ans_threshold(
        precision_raw, na_probs, qid_to_has_ans, OPTS.na_prob_thresh
    )
    recall_thresh = apply_no_ans_threshold(
        recall_raw, na_probs, qid_to_has_ans, OPTS.na_prob_thresh
    )

    out_eval = make_eval_dict(exact_thresh, f1_thresh, precision_thresh, recall_thresh)
    if has_ans_qids:
