...
from mlm_utils import DataCollatorForEntityLanguageModeling
...
class ModelArguments:
    ...
    config_overrides: Optional[str] = field(
        default=None,
        metadata={
            "help": "Override some existing default config settings when a model is trained from"
            "scratch. Example: n_embd=10,resid_pdrop=0.2,scale_attn_weights=false,"
...
class DataTrainingArguments:
    ...
    dataset_type: Optional[str] = field(
        default="expected",
        metadata={"help": "The name of the dataset to use (default: 'expected')."},
    )
    dataset_path: Optional[str] = field(
        default="../data",
        metadata={"help": "Set path to dataset directory (default: '../data')."},
    )
    gpu_device: Optional[int] = field(
        default=2,
        metadata={"help": "Set the GPU DEVICE number (default: 2)."},
    )
    mask_probability: Optional[float] = field(
        default=0.8,
        metadata={
            "help": "Probability that a word is replaced by a <mask> token. The probabilities for "
            "replacing of the the token with a random token and the keeping of the token are "
            "calculated with this formula: $(1-mask_probability)/2$ (default: 0.8)."
        },
    )
    send_extra_message: str = field(
        default="",
        metadata={
            "help": "Sends an additional custom message within the mail (default: None)."
        },
    )
    send_mail: bool = field(
        default=False,
        metadata={
            "help": "Sends an email after the training is done. 'From' and 'to' has to be "
            "specified in a `mail.json` file in 'dataset_path' with the key 'email' and the password "
            "with the key 'password'  (default: False)."
        },
    )
    use_entities_for_masking: str = field(
        default="",
        metadata={
            "help": "Use entity masking within the data collator by giving the path to an "
            "entites.txt file (default: '')."
        },
    )
    ...
    validation_split_percentage: Optional[int] = field(
        default=5,
        metadata={
            "help": "The percentage of the train set used as validation set in case there's"
            "no validation split"
        },
    )
...
def main(parser):
    ...
    if not training_args.no_cuda:
        ...
        os.environ["CUDA_VISIBLE_DEVICES"] = str(data_args.gpu_device)

    # Setup logging
    Path("logs").mkdir(parents=True, exist_ok=True)
    logging_filename = f"logs/mlm_{data_args.dataset_type}.log"
    logging.basicConfig(
    ...
    # Detecting last checkpoint. Only if in train mode (= do_train)
    ...
    if (
        ...
        if last_checkpoint is None and len(os.listdir(training_args.output_dir)) > 0:
            ...
            logger.info(
                ...
                "behavior, change the `--output_dir` or add `--overwrite_output_dir` "
    ...
    if model_args.tokenizer_name:
        ...
        raise ValueError(
            ...
            "script. You can do it from another script, save it, and load it from here, using "
            "--tokenizer_name."
        )
    ...
    if data_args.max_seq_length is None:
        ...
        if max_seq_length > 1024:
            logger.warning(
                ...
                "default value by passing --max_seq_length xxx."
            )
            max_seq_length = 1024
    else:
        if data_args.max_seq_length > tokenizer.model_max_length:
            logger.warning(
                ...
                f"maximum length for the model ({tokenizer.model_max_length}). "
    ...
    if data_args.line_by_line:
        ...
        # Otherwise, we tokenize every text, then concatenate them together before splitting them
        # in smaller parts. We use `return_special_tokens_mask=True` because
        # DataCollatorForLanguageModeling (see below) is more efficient when it receives the
        # `special_tokens_mask`.
        def tokenize_function(examples):
            return tokenizer(
    ...
    if data_args.use_entities_for_masking:
        data_collator = DataCollatorForEntityLanguageModeling(
            entities_file_path=data_args.use_entities_for_masking,
    ...
    if data_args.send_mail:
        import yagmail

        splitted_val_metrics = "\n".join([f"{k}: {v}" for k, v in metrics.items()])

        with open(f"{data_args.dataset_path}/mail.json", "r") as f:
            subject_name = f"MLM: {data_args.dataset_type}"
            f" ({datetime.now():%d.%m.%y}_{datetime.now():%H:%M})"
            mail = json.load(f)
            yag = yagmail.SMTP(mail["email"], mail["password"])
            contents = f"MLM: {data_args.dataset_type}\n\n"
            contents += str(data_args.send_extra_message)
            contents += "\n\n"
            contents += f"{splitted_val_metrics}\n\n"
            contents += f"Val metrics: {metrics}\n"
